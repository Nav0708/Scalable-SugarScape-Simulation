...
def runSimulations(config, configFiles, path):
    ...
    for conf in configFiles:
        baseName = os.path.splitext(conf)[0]
        baseName = baseName.split("/")[1]
        outputFileName = f"{baseName}.json"
        conf = f"{baseName}.config"

        makeflowContent += (f"{outputFileName}: {conf} sugarscape.py agent.py cell.py disease.py environment.py ethics.py\n"
                            f"\t python3 sugarscape.py --conf {conf} > {outputFileName}\n\n")

    with open("runSimulations.mf", "w") as mfFile:
        mfFile.write(makeflowContent)
